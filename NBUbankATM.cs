...
{
    public class NBUbankATM : ILogin, IBalance, IDeposit, IWithdrawal, IThirdPartyTransfer, ITransaction
    {
        ...
        public void Execute()
        {
            ...
            while (true)
            {
                switch (Utility.GetValidIntInputAmt("your option"))
                {
                        ...
                        while (true)
                        {
                            ...
                            switch (Utility.GetValidIntInputAmt("your option"))
                            {
                                ...
                                case (int)SecureMenu.PlaceDeposit:
                                    PlaceDepositConsole(selectedAccount);
                                    break;
                                ...
                                case (int)SecureMenu.ThirdPartyTransfer:
                                    ...
                                    PerformThirdPartyTransferConsole(selectedAccount, vMThirdPartyTransfer);
                                    break;
                    ...
                    case 2:
                        Console.Write("\nThank you for using NBUBank. Exiting program now .");
        ...
        public void InitializeListofTransactions()
        {
        ...
        private static void LockAccount()
        {
            ...
            Console.WriteLine("Thank you for using NBUBank. ");
            Console.ReadKey();
            System.Environment.Exit(1);
        }
        ...
        public void Initialization()
        {
            ...
            {
                new BankAccount() { FullName = "Azim", AccountNumber=333111, CardNumber = 123, PinCode = 111111, Balance = 2000.00m, isLocked = false },
                new BankAccount() { FullName = "Abdumalik", AccountNumber=111222, CardNumber = 456, PinCode = 111111, Balance = 1500.30m, isLocked = true },
                new BankAccount() { FullName = "Toshmat", AccountNumber=888555, CardNumber = 789, PinCode = 111111, Balance = 2900.12m, isLocked = false }
            };
        }
        ...
        public bool PlaceDeposit(BankAccount account, decimal amt)
        {
        ...
        public void PlaceDepositConsole(BankAccount account)
        {
        ...
        public void MakeWithdrawal(BankAccount account)
        {
            ...
            //******  TODO for testing commented out checks.  ****
            //Fix 1
            ...
            {
                // Transaction transaction = CreateNewTransaction(account);
                //  InsertTransaction(account, transaction);
                // Add transaction record - End

                // WithdrawalUpdate(account);
                MakeActualWithdrawal(account, transaction_amt);

                Utility.PrintMessage($"Please collect your money. You have successfully withdraw {Utility.FormatAmount(transaction_amt)}", true);
            }
        }

        public void MakeActualWithdrawal(BankAccount account, decimal amt)
        {
            Transaction transaction = CreateNewTransaction(account, amt);
            InsertTransaction(account, transaction);
            WithdrawalUpdate(account);
        }

        private static Transaction CreateNewTransaction(BankAccount account, decimal amt)
        {
            // Bind transaction_amt to Transaction object
            // Add transaction record - Start
        ...
        private static Transaction CreateNewTransaction(BankAccount account)
        {
            // Bind transaction_amt to Transaction object
            // Add transaction record - Start
            return new Transaction()
        ...
        private static void WithdrawalUpdate(BankAccount account)
        {
            // Another method to update account balance.
        ...
        public bool PerformThirdPartyTransfer(BankAccount bankAccount, VMThirdPartyTransfer vMThirdPartyTransfer)
        {
            ...
            else
            {
                ...
                if (selectedBankAccountReceiver == null)
                    ...
                    throw new AccountNameMissmatchException();
                else
                {
                    // Bind transaction_amt to Transaction object
                    // Add transaction record - Start
        ...
        public void PerformThirdPartyTransferConsole(BankAccount bankAccount, VMThirdPartyTransfer vMThirdPartyTransfer)
        {
            if (vMThirdPartyTransfer.TransferAmount <= 0)
                Utility.PrintMessage("Amount needs to be more than zero. Try again.", false);
            else if (vMThirdPartyTransfer.TransferAmount > bankAccount.Balance)
